...
describe("OtaService", (): void => {
    ...
    beforeEach((): void => {
        ...
        valueChangedListenerReady = async (broadcastValue?: DataView, runCount: number = 1) => {
            let callCount = 1;

            for (; callCount <= runCount; ) {
                await new Promise((resolve: (value: { callCount: number }) => void) => {
                    mockResponseCharacteristic.addEventListener = jasmine
        ...
        mockResponseCharacteristic.removeEventListener = jasmine
    ...
    describe("abortOta method", (): void => {
        beforeEach(async (): Promise<void> => {
            ...
            service.performOta(testFile).catch(() => {
                // no-op
            });
