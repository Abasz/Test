...
export class TileGridHelper {
    ...
    static areTilePositionsEqual(tileA: TilePosition, tileB: TilePosition): boolean {
        return (
            tileA.rowStart === tileB.rowStart &&
            tileA.columnStart === tileB.columnStart &&
            tileA.rowSpan === tileB.rowSpan &&
            tileA.columnSpan === tileB.columnSpan
        );
    }
    ...
    static findSwapPositions(
        ...
        for (const pos of [targetPosition, ...remainingPositions]) {
    ...
    private static findPositionInArea(
        ...
        const maxRow = area.rowStart + area.rowSpan - rowSpan;
        const maxCol = area.columnStart + area.columnSpan - columnSpan;

        for (let r = area.rowStart; r <= maxRow; r++) {
            for (let c = area.columnStart; c <= maxCol; c++) {
                let canFit = true;
                for (let dr = 0; dr < rowSpan && canFit; dr++) {
                    for (let dc = 0; dc < columnSpan && canFit; dc++) {
                        if (occupiedCells.has(`${r + dr}-${c + dc}`)) {
                            canFit = false;
                        }
                    }
                }
                if (canFit) {
                    return { rowStart: r, columnStart: c, rowSpan, columnSpan };
                }
            }
        }
