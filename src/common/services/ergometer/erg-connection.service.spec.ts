...
describe("ErgConnectionService", (): void => {
    ...
    const createMockBluetoothDevice = (overrides: Partial<BluetoothDevice> = {}): BluetoothDevice => {
        let baseBluetoothDevice: Partial<BluetoothDevice> = {
            id: "mock-device-id",
            name: "Mock Ergo",
            watchAdvertisements: jasmine.createSpy("watchAdvertisements").and.returnValue(Promise.resolve()),
        };
    ...
    const setupConnectionSpies = (): typeof connectionSpies => {
        return {
            connectToMeasurement: spyOn(ergConnectionService, "connectToMeasurement").and.returnValue(
                Promise.resolve(),
            ),
            connectToExtended: spyOn(ergConnectionService, "connectToExtended").and.returnValue(
                Promise.resolve(),
            ),
            connectToHandleForces: spyOn(ergConnectionService, "connectToHandleForces").and.returnValue(
                Promise.resolve(),
            ),
            connectToDeltaTimes: spyOn(ergConnectionService, "connectToDeltaTimes").and.returnValue(
                Promise.resolve(),
            ),
            connectToSettings: spyOn(ergConnectionService, "connectToSettings").and.returnValue(
                Promise.resolve(),
            ),
            connectToStrokeSettings: spyOn(ergConnectionService, "connectToStrokeSettings").and.returnValue(
                Promise.resolve(),
            ),
            connectToBattery: spyOn(ergConnectionService, "connectToBattery").and.returnValue(
                Promise.resolve(),
            ),
        };
    };
    ...
    it("should instantiate and expose initial 'disconnected' status", (): void => {
        ...
        expect(localStatusEvents.length).toBe(1);
        ...
        it("should retry reconnect when watchAdvertisements throws", async (): Promise<void> => {
            const mockDeviceWithError = createMockBluetoothDevice({
                watchAdvertisements: jasmine
                    ...
                    .and.returnValue(Promise.reject(new Error("watch failed"))),
            });
            navigatorSpy.and.returnValue({
                bluetooth: {
    ...
    describe("connection flow", (): void => {
        ...
        describe("on successful connection", (): void => {
            it("should call all connectTo* methods", async (): Promise<void> => {
                ...
                expect(connectionOrder.length).toBe(7);
        ...
        describe("on connection failure", (): void => {
            it("should set disconnected status when connection error occurs", async (): Promise<void> => {
                connectionSpies.connectToMeasurement.and.returnValue(
            ...
            it("should handle gatt.connect returning falsy value", async (): Promise<void> => {
                const mockFailedDevice = createMockBluetoothDevice({
                    gatt: {
                        connected: true,
                        connect: jasmine.createSpy("connect").and.returnValue(Promise.resolve(undefined)),
                    } as unknown as BluetoothRemoteGATTServer,
                });
                ...
                const mockGattWithError = createMockBluetoothDevice({
                    gatt: {
                        ...
                        getPrimaryService: jasmine
                            ...
                            .and.returnValue(Promise.reject(new Error("Service unavailable - test"))),
                    } as unknown as BluetoothRemoteGATTServer,
