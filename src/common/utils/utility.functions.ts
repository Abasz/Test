...
export function upperQuartile(data: Array<number>): number {
    if (data.length < 4) {
        throw new Error("Data must be an array with at least 4 numbers.");
    }
    ...
    const upperHalf =
        sortedData.length % 2 === 0 ? sortedData.slice(midIndex) : sortedData.slice(midIndex + 1);

    const q3MidIndex = Math.floor(upperHalf.length / 2);

    const q3 = upperHalf.length % 2 === 0 ? upperHalf.slice(q3MidIndex) : upperHalf.slice(q3MidIndex + 1);

    return q3.reduce((accumulator: number, current: number): number => accumulator + current, 0) / q3.length;
}
