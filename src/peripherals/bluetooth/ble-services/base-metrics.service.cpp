...
{
    {
        ...
        const auto strokeRate = static_cast<unsigned char>(std::lroundl((params->data.strokeCount - params->data.previousStrokeCount) / ((params->data.strokeTime - params->data.previousStrokeTime) / secInMicroSec / 60U)));
        const auto pace500m = static_cast<unsigned short>(std::lroundl(500U / (((params->data.distance - params->data.previousDistance) / 100U) / ((params->data.revTime - params->data.previousRevTime) / secInMicroSec))));
