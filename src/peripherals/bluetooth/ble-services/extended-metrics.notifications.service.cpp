...
void ExtendedMetricBleService::HandleForcesParams::task(void *parameters)
{
    ...
    std::span<const float> floatView{params->handleForces};
    std::span<const std::byte> byteView = std::as_bytes(floatView);
    ...
    for (size_t i = 0; i < split; ++i)
    {
        ...
        const size_t offset = i * chunkSizeInBytes;
        const size_t count = std::min(chunkSizeInBytes, totalBytes - offset);

        buffer.push_back(static_cast<unsigned char>(split));
        buffer.push_back(static_cast<unsigned char>(i + 1));

        const size_t headerSize = buffer.size();
        buffer.resize(headerSize + count);

        std::memcpy(&buffer[headerSize], &byteView[offset], count);

        params->characteristic->setValue(buffer.data(), buffer.size());
        params->characteristic->notify();
    }
    vTaskDelete(nullptr);
}
