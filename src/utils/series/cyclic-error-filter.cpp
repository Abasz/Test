...
void CyclicErrorFilter::updateFilter(const unsigned long position, const Configurations::precision rawValue, const Configurations::precision cleanValue)
{
    ...
    // Log slot tracker activity when boost is applied
    if (boost > static_cast<Configurations::precision>(1.0))
    {
        const auto median = slotErrorTrackers[slot].median();
        const auto meanSign = slotErrorTrackers[slot].meanSign();
        const auto rawR2 = rawOlsSeries.size() > 0 ? rawOlsSeries.goodnessOfFit() : 0.0;
        const auto cleanR2 = cleanOlsSeries.size() > 0 ? cleanOlsSeries.goodnessOfFit() : 0.0;
        // CSV format: [CYCLIC],SLOT_TRACKER,slot,median,meanSign,boost,signedDev,rawR2,cleanR2,goodnessOfFit
        printf("[CYCLIC],SLOT_TRACKER,%u,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f\n",
               slot, median, meanSign, boost, signedDev, rawR2, cleanR2, goodnessOfFit);
    }
...
bool CyclicErrorFilter::isPotentiallyMisaligned()
{
    ...
    const auto ratio = (rawR2 > 0) ? cleanR2 / rawR2 : 1.0;
    ...
    {
        // CSV format: [CYCLIC],RECOVERY_CHECK,rawR2,cleanR2,ratio,triggered,decayFactor
        printf("[CYCLIC],RECOVERY_CHECK,%.6f,%.6f,%.6f,0,0\n", rawR2, cleanR2, ratio);
        return false;
    }
    ...
    // CSV format: [CYCLIC],RECOVERY_CHECK,rawR2,cleanR2,ratio,triggered,decayFactor
    printf("[CYCLIC],RECOVERY_CHECK,%.6f,%.6f,%.6f,1,%.4f\n", rawR2, cleanR2, ratio, decayFactor);
    // CSV format: [CYCLIC],GLOBAL_DECAY,rawR2,cleanR2,decayFactor
    printf("[CYCLIC],GLOBAL_DECAY,%.6f,%.6f,%.4f\n", rawR2, cleanR2, decayFactor);
