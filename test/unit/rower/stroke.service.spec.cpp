        ...
        for (const auto &deltaTime : deltaTimes)
        {
            ...
            if (rowingMetrics.strokeCount > prevStrokeCount)
            {
                SECTION("force curves on new stroke (" + std::to_string(rowingMetrics.strokeCount) + ")")
                {
                    ...
                    CHECK_THAT(rowingMetrics.driveHandleForces, Catch::Matchers::RangeEquals(forceCurves.at(rowingMetrics.strokeCount - 1), [](float first, float second)
                                                                                             { return std::abs(first - second) < 0.00001F; }));
                    CHECK_THAT(rowingMetrics.dragCoefficient * 1e6, Catch::Matchers::WithinRel(dragFactors.at(rowingMetrics.strokeCount - 1), 0.0000001));
                }
            }
        }
